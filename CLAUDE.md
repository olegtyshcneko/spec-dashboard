...
For tagged releases (`git tag v<x.y.z>`, "Release v<x.y.z> …" commit) additionally update the tag-pinned references: `plugins/spec-dashboard/.codex-plugin/plugin.json`, the `--package` ref in `plugins/spec-dashboard/.mcp.json`, and the `--ref` in the README quickstart.
